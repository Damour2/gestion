...
?>
	...
	<div class='recuadro'>
        ...
        <fieldset>
            ...
            <p>
            	<label>Margen</label><input type="text" id="margen_recomendado" class='number' name="margen_recomendado" value="<?=(isset($r[margen_recomendado])) ? $r[margen_recomendado] : 300 ?>"/>
				...
				<input type="radio" name="auto_publico" id="auto_publico_no" <?=(isset($r[auto_publico]) and  $r[auto_publico] != 1) ? 'checked="checked"' : '' ?>/><label>NO</label>
            </p>
            <p><label>Precio al público</label><input type="text" name="valor" class='number'id="valor" value="<?=$r[valor]?>" /></p>
         </fieldset> 
         
         <fieldset>
        	<legend>Precio para revendedores</legend>
            <p>
            	<label>Margen</label><input type="text" id="margen_mayor" class='number' name="margen_mayor" value="<?=(isset($r[margen_mayor])) ? $r[margen_mayor] : 100?>"/>
                ...
                <input type="radio" name="auto_mayor" id="auto_mayor_no" <?=(isset($r[auto_mayor]) and  $r[auto_mayor] != 1) ? 'checked="checked"' : '' ?>/><label>NO</label>
            </p>
            <p><label>Precio por mayor</label><input type="text" class='number' name="valor_mayor" id="valor_mayor" value="<?=$r[valor_mayor]?>" /></p>
         </fieldset>   	
	...
	<div id="datos_generales">
        ...
        <div id="materias_div">
					...
					if (count($materias->results))
						foreach( $materias->results as $rm)
							echo "<div class='linea'>
									...
									<input type='text' class='cantidad number'  placeholder='Cantidad' value='$rm[cantidad]'/>
		    ...
		    ?>
            <div class="linea">
            	<input type="text" class="nombre" /><label>Unidad => $ <span class="valor_unitario">0.00</span></label><input type="text" class="cantidad number" placeholder="Cantidad" /><span class="valor_costo">$ <em>0.00</em></span>
                <img src="img/borrar.png" title="Eliminar" onclick="eliminar(this)" />
              </div>  
        ...
        <div id="variante_div">
        	...
        	<? 	if ($id) {
					$temp = new DB();
					$materias = $temp->select("variantes", 'id_var, variante, stock, stock_minimo', "id_art = $id");
					if (count($materias->results))
						foreach( $materias->results as $rm)
							echo "<div class='linea' id='$rm[id_var]'>
            						<input type='text' class='variante' value='$rm[variante]' />
									<label>Stock</label>
									<input type='text' class='stock number' value='$rm[stock]'/>
									<label>Stock Minimo</label>
									<input type='text' class='stock_minimo number' value='$rm[stock_minimo]'/>
    		            			<img src='img/borrar.png' title='Eliminar' onclick='eliminar(this)' />
	              				</div>"; 
				}
				
		    ?>
            <div class='linea' id='0'>
            						<input type='text' class='variante' value='' placeholder='Variante' />
									<label>Stock</label>
									<input type='text' class='stock number' value=''/>
									<label>Stock Minimo</label>
									<input type='text' class='stock_minimo number' value=''/>
    		            			<img src='img/borrar.png' title='Eliminar' onclick='eliminar(this)' />
	              				</div>
              <a class="boton insertar">AGREGAR NUEVA VARIANTE</a>
        </div>
...
$(document).ready(function() {
	...
	nuevaMateria = '<div class="linea">            	<input type="text" class="nombre" /><label>Unidad => $ <span class="valor_unitario">0.00</span></label><input type="text" class="cantidad number" placeholder="Cantidad" /><span class="valor_costo">$ <em>0.00</em></span>                <img src="img/borrar.png" title="Eliminar" onclick="eliminar(this)" />           </div>';


	nuevaVariante = "<div class='linea' id='0'><input type='text' class='variante' value='' placeholder='Variante' />									<label>Stock</label>									<input type='text' class='stock number' value=''/>									<label>Stock Minimo</label>									<input type='text' class='stock_minimo number' value=''/>    		            			<img src='img/borrar.png' title='Eliminar' onclick='eliminar(this)' />	              				</div>";
	...
	*/ ?>
	...
	actualizarImpares("#imagenes_art");
		...
		$('#variante_div .insertar').click(function() {
			$(this).before(nuevaVariante); //agrega una nueva variante
		});
		...
		$('input#margen_mayor, input#margen_recomendado').change(function() {
			chequearAutomaticos();
		});
...
function guardar_articulo(reloadear) {
	...
	datos = { articulo : <?=$id?>,
			  ...
			  variantes : (function() {
			  		var variantes = []; 
			  		$('#variante_div').find('.linea').each(function() {
						$this = $(this);
						if ($this.attr('id') != 0 || $this.find('input.variante').val() != '')
							variantes.push( 
								{ id_var: $this.attr('id'),
								  variante : $this.find('.variante').val(),
								  stock:  $this.find('.stock').val(),
								  stock_minimo : $this.find('.stock_minimo').val()
								  });
					});
